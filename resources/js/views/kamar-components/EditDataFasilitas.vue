<template>
  <div>
      <form ref="form" enctype="multipart/form-data">
          ...
          <b-button variant="outline-primary" size="sm" @click="update">Edit Fasilitas</b-button>
...
export default {
    ...
    methods: {
        getDataFasilitas(id) {
            axios(`/api/v1/fasilitas-kamar/${id}`)
                .then(response => {
                    let { result } = response.data
                    let { nama_fasilitas_kamar } = result
                    console.log('response', response)
                    this.form.nama_fasilitas_kamar = nama_fasilitas_kamar
                })
                .catch(e => {
                    console.log(e)
                })
        ...
        update() {
            ...
            axios.put(`/api/v1/fasilitas-kamar/${this.form.kamar_id}`, this.form)
                    .then(response => {
                        let { result } = response.data
                        console.log(result)
                        ...
                        setTimeout(() => { this.$router.push({ name: 'admin-kamar-fasilitas', params: {id: this.kamar_id }}) }, 2000)
                    })
                    .catch(e => {
